...
#define FUNCTION_PARAM_ADDR_OFFSET      0x200
    ...
    if (ptrace_call(target_pid, (uint32_t)hook_entry_addr, parameters, 1,  &regs) == -1)
        goto exit;
...
int main(int argc, char** argv) {
    pid_t target_pid;
    target_pid = find_pid_of("./loop");
    //target_pid = find_pid_of("/data/test");
	printf("pid=%d\n",target_pid);
    inject_remote_process(target_pid, "/home/user1/injectso/hook/hello.so", "hook_entry",  "I'm parameter!", strlen("I'm parameter!"));
}